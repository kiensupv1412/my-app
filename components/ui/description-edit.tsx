...
export type InlineEditorHandle = {
    getHtml: () => string;
    getText: () => string;
    setHtml?: (v: string) => void; // optional
};

type InlineEditorProps = {
    description?: string; // HTML khởi tạo
};
const InlineEditor = React.forwardRef<InlineEditorHandle, InlineEditorProps>(
    ({ description = '' }, ref) => {
        // ----- Refs -----
        ...
        // Set nội dung KHỞI TẠO 1 lần (tránh React ghi đè khi đang gõ)
        React.useEffect(() => {
        ...
        React.useImperativeHandle(ref, () => ({
            ...
            setHtml: (v: string) => { if (editorRef.current) editorRef.current.innerHTML = v; },
        }));
